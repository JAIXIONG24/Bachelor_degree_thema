...
<form action="k_region2_indi_delete.php" method="POST">
  <input type="hidden" name="uid" value="<?php echo $uid; ?>">
  <input type="hidden" name="na" value="<?php echo $na; ?>">

  <input type="hidden" name="userid" value="<?php echo $id; ?>">
  <input type="hidden" name="username" value="<?php echo $n; ?>">
  <input type="submit" name="henkou" value="削除" class="btn btn-danger">
</form>
<?php

}else{
  $query2="select * from User_tabuken left join Indi_tabuken on User_tabuken.date=Indi_tabuken.date left join Pra_tabuken on Indi_tabuken.praid=Pra_tabuken.praid where userid='$uid' and kind=2 and time='$t'";
...
<form action="k_region2_henkoukakunin.php" method="POST"><!--実習先登録ボタン-->
  <p>地域創生2の学生</p>
  <p>ID：<?php echo $uid ?>　<?php echo $na ?>さん</p>
  <br>
  <p>実習名：<?php echo $pn; ?></p>
  <p>日付：<input type="datetime" name="time" value="<?php echo $t; ?>"></p>
...
<form action="k_region2_indi.php" method="post"><!--管理者登録ボタン-->
  <input type="hidden" name="uid" value="<?php echo $uid; ?>">
