...
$query="select count(*) as sum from User_tabuken where userid like '$r' and kind=1";
...
<?php
  ...
  $query5="select * from User_tabuken where userid like '$uid' and kind=1";
...
<form action="k_region1_henkou.php" method="POST"><!--検索ボタン-->
...
<form action="k_region1_indi_regist.php" method="POST"><!--検索ボタン-->
  <input type="hidden" name="uid" value="<?php echo $uid; ?>">
  <input type="hidden" name="na" value="<?php echo $na; ?>">
...
$query1="select praid,count(*) as ppp from User_tabuken natural join Indi_tabuken where userid='$uid' && kind=1 group by praid";
...
if(isset($oo)){
  ...
  echo "<tr><th>日付</th><th>実習内容</th><th>実習時間</th><th></th></tr>";
       ...
       $query2="select * from User_tabuken left join Indi_tabuken on User_tabuken.date=Indi_tabuken.date left join Pra_tabuken on Indi_tabuken.praid=Pra_tabuken.praid where userid='$uid' and kind=1 ";
       $stmt2 = $dbh->query($query2);
       $rec2 = $stmt2->fetchAll(PDO::FETCH_ASSOC);
       ...
       foreach($rec2 as $row2 ){
        ...
        echo "<td>$pt</td>";
        echo "<td>";
        echo "<form action='k_region1_henkou.php' method='post'>";
